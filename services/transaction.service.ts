...
export class TransactionService {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      for (const item of data.tickets) {
        ...
        if (ticket.available < item.quantity)
          throw new Error(`Not enough seats for ticket: ${ticket.name}`);
        if (item.quantity > ticket.maxPerOrder)
          throw new Error(
            `Max ${ticket.maxPerOrder} tickets per order for: ${ticket.name}`,
          );

        const itemSubtotal =
          parseFloat(ticket.price.toString()) * item.quantity;
      ...
      if (data.voucherCode) {
        ...
        if (
          voucher.minPurchase &&
          subtotal < parseFloat(voucher.minPurchase.toString())
        ) {
          throw new Error(
            `Minimum purchase of Rp ${voucher.minPurchase} required`,
          );
        }

        if (voucher.isPercent) {
          voucherDiscount =
            subtotal * (parseFloat(voucher.discount.toString()) / 100);
          if (voucher.maxDiscount)
            voucherDiscount = Math.min(
              voucherDiscount,
              parseFloat(voucher.maxDiscount.toString()),
            );
        } else {
      ...
      if (data.pointsToUse > 0) {
        ...
        if (data.pointsToUse > totalAvailablePoints)
  ...
  async cancelTransaction(transactionId: string, userId: string) {
    const transaction = await prisma.transaction.findFirst({
      where: {
        id: transactionId,
        userId,
        status: { in: ["PENDING_PAYMENT", "WAITING_CONFIRMATION"] },
      },
    });
    if (!transaction)
  ...
  private async cancelTransactionAndRestore(
    transactionId: string,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const totalSeats = transaction.details.reduce(
        (sum, d) => sum + d.quantity,
        0,
      );
      await tx.event.update({
